...
run "plan" {
  ...
  assert {
    condition      = azurerm_key_vault.keyvault.network_acls[0].virtual_network_subnet_ids == null
    error_message  = "kv network_acls.virtual_network_subnet_ids must be set to null by default"
  }
